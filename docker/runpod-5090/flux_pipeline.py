...
class FluxPipeline:
    ...
    @torch.inference_mode()
    def into_individual_bytes(
        self, x: torch.Tensor, jpeg_quality: int = 99
    ) -> List[io.BytesIO]:
        """Converts a batch image tensor to a list of individual BytesIO JPEG buffers."""
        num_images = x.shape[0]
        results: List[io.BytesIO] = []
        for i in range(num_images):
            single = (
                x[i]
                .clamp(-1, 1)
            ...
            results.append(self.img_encoder.encode_torch(single, quality=jpeg_quality))
        return results
    ...
    def generate(
        ...
        if return_individual:
            return self.into_individual_bytes(img, jpeg_quality=jpeg_quality), seed
        if return_seed:
